...
@app.route('/preprocess_dataset', methods=['GET'])
def upload_form():
    return render_template('Preprocess Dataset.html')
...
def preprocess_dataset():
    try:
        ...
        if 'Contextcj' not in df.columns:
            os.remove(file_path)
            return jsonify({'error': 'Required column "Contextcj" not found in the uploaded file.'}), 400

        preprocessed_data = []
        for text in df['Contextcj']:
        ...
        os.remove(file_path)
        return jsonify({'message': 'Dataset preprocessed successfully.', 'data': preprocessed_data})

    except Exception as e:
        logging.error(f"Error in /preprocess_dataset: {e}")
